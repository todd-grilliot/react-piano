...
const KeyBoard = ({ keyNameToggle }) => {
    ...
    useEffect(() => {
        if (prevNotesArePressed.current) {
            for (let i = 0; i < notesArePressed.length; i++) {
                if (
                    !notesArePressed[i].nat &&
                    prevNotesArePressed.current[i].nat
                ) {
                    stopNote(i, true);
                }
                if (
                    !notesArePressed[i].sharp &&
                    prevNotesArePressed.current[i].sharp
                ) {
                    stopNote(i, false);
                }
            }
        ...
        if (
            notesArePressed.every(
                (value) => value.nat === false && value.sharp === false
            )
        ) {
            stopAllNotes();
        }
    ...
    //constructing keyComponentArray for rendering...
